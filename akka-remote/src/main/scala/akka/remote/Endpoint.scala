...
private[remote] class EndpointWriter(
  ...
  whenUnhandled {
    ...
    case Event(TakeOver(newHandle), _) ⇒
      handle foreach { _.disassociate() } // Signal to shut down old reader
...
private[remote] final class EndpointReader(
  ...
  override def receive: Receive = {
    ...
    case Disassociated ⇒ self forward PoisonPill // Minimizze the window of opportunity of more reads by doing a final "flush"
  }
