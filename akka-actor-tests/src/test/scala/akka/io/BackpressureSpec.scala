...
object BackpressureSpec {
  ...
  case class ReceiverStatus(restarted: Throwable, received: Long, hiccupStarted: Int, hiccupEnded: Int)
  ...
  class Receiver(hiccups: Boolean) extends Actor with Stash with ActorLogging {
    ...
    override def unhandled(msg: Any): Unit = msg match {
      case GetStatus ⇒ sender ! ReceiverStatus(restarted, received, hiccupStarted, hiccupEnded)
    }
